...
SkyMed is the command, coordination, and intelligence software layer for a VTOL medical drone network serving tribal Andhra Pradesh. It is an offline-first AI triage priority scoring system, drone dispatch coordination platform, and telemetry relay linking ASHA workers to AMTZ doctors. It does not diagnose, does not transport infectious biological samples, and does not dispatch drones without doctor confirmation.

## Architecture

SkyMed has three layers:

- Drone Edge: rugged Android tablet, Bluetooth vitals, local INT8 triage priority scoring, symptom photo capture.
...
8. Confirm the response shows `P1`, score, flagged vitals, suggested heat-stable payload, and drone ETA.
9. Toggle `Doctor Confirmed`.
10. Click `Dispatch Drone`, select payload items, and confirm the payload stays under 1.5kg.
11. Confirm a mission ID appears, then open Drone Fleet to watch route, status stepper, and battery telemetry.
12. Return to the navbar and toggle Offline Mode.
13. Submit another triage case while offline.
14. Confirm the UI switches to Local Edge Mode and shows `Sync Queue: 1 Pending`.
15. Toggle back online and watch the animated flush from ASHA tablet to village mesh to district cloud.
